...
uint32_t bootloader_get_active_addr(void){
	flash_read_shared_info();
	return shared_info.active_app_addr;
}
...
void bootloader_executeapp(void){
	pc_adjust pc;
	uint32_t addr = shared_info.active_app_addr;
...
void bootloader_updateapp(void){
	if(shared_info.active_app_addr == APP1_ADDR){
		shared_info.active_app_addr = APP2_ADDR;
	}
	else{
		shared_info.active_app_addr = APP1_ADDR;
	}
	LOG_DEBUG(TAG, "new partition is 0x%08x", shared_info.active_app_addr);
